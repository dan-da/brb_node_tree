...
type State = BRBTree<Actor, TypeId, Meta>;
...
impl SharedBRB {
    ...
    fn opmove(&mut self, parent_id: TypeId, meta: Meta, child_id: TypeId) -> Vec<Packet> {
        let op = {
            self.brb
                .lock()
                .unwrap()
    ...
    fn apply(&mut self, packet: Packet) -> Vec<Packet> {
...
impl Repl {
    ...
    fn mv(&mut self, args: &[String]) -> CommandResult {
        ...
        let parent_id = args[0].parse::<TypeId>().unwrap();
        let meta = args[1].parse::<Meta>().unwrap();
        let child_id = args[2].parse::<TypeId>().unwrap();

        for packet in self.state.opmove(parent_id, meta, child_id) {
            self.network_tx
